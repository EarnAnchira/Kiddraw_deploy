...
const ImageComponent = () => {
    ...
    const fetchAudioAnswerEng1 = async () => {
        const response = await fetch(`/audioanswereng1/${StoryID}/${PageNoAnswer1}`);
        const blob = await response.blob();
        setAudioAnswerEng1(blob);
    };

    const fetchAudioAnswerEng2 = async () => {
        const response = await fetch(`/audioanswereng2/${StoryID}/${PageNoAnswer1}`);
        const blob = await response.blob();
        setAudioAnswerEng2(blob);
    };

    const fetchAudioEng = async () => {
        const response = await fetch(`/audioeng/${StoryID}/${PageNoAnswer1}`);
        const blob = await response.blob();
        setAudioEng(blob);
    };

    const fetchAudioThai = async () => {
        const response = await fetch(`/audiothai/${StoryID}/${PageNoAnswer1}`);
        const blob = await response.blob();
        setAudioThai(blob);
    };
    ...
    const getDataDetail = async () => {
        const res = await fetch(`/poseanimatorD/${StoryID}/${PageNoNext}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json"
            }
